...
(* FIXME: duplicated in globals.ml >.< *)
let string_of_pos (pos:Globals.loc) =
  let open Globals in
...
let pprint msg (pos:Globals.loc) =
  let open Globals in
  let tmp = pos.start_pos.Lexing.pos_fname ^ ":" ^ (string_of_int pos.start_pos.Lexing.pos_lnum) ^ ": "^ (string_of_int (pos.start_pos.Lexing.pos_cnum-pos.start_pos.Lexing.pos_bol))^ ": " ^ msg in print tmp
...
let devel_pprint (msg:string) (pos:Globals.loc) =
  let flag = !devel_debug_on in
  ho_print flag (fun m -> (prior_msg pos)^m) msg

let devel_hprint (pr:'a->string) (m:'a) (pos:Globals.loc) = 
  let flag = !devel_debug_on in
  ho_print flag (fun x -> (prior_msg pos)^(pr x)) m

let devel_zprint msg (pos:Globals.loc) =
  let flag = !devel_debug_on in
...
let binfo_start (msg:string) =
        binfo_pprint "**********************************" Globals.no_pos;
        binfo_pprint ("**** "^msg^" ****") Globals.no_pos;
        binfo_pprint "**********************************" Globals.no_pos

let binfo_end (msg:string) =
        binfo_pprint "**********************************" Globals.no_pos;
        binfo_pprint ("**** end of "^msg^" ****") Globals.no_pos;
        binfo_pprint "**********************************" Globals.no_pos

let dinfo_start (msg:string) =
        dinfo_pprint "**********************************" Globals.no_pos;
        dinfo_pprint ("**** "^msg^" detected ****") Globals.no_pos;
        dinfo_pprint "**********************************" Globals.no_pos

let dinfo_end (msg:string) =
        dinfo_pprint "**********************************" Globals.no_pos;
        dinfo_pprint ("**** end of "^msg^" ****") Globals.no_pos;
        dinfo_pprint "**********************************" Globals.no_pos
...
let trace_pprint (msg:string) (pos:Globals.loc) : unit = 
	ho_print false (fun a -> " "^a) msg

let trace_hprint (pr:'a->string) (m:'a) (pos:Globals.loc) = 
	ho_print false (fun x -> " "^(pr x)) m

let trace_zprint m (pos:Globals.loc) = 
...
let info_pprint (msg:string) (pos:Globals.loc) : unit = 
	ho_print true (fun a -> " "^a) msg

let info_hprint (pr:'a->string) (m:'a) (pos:Globals.loc) = 
	ho_print true (fun x -> " "^(pr x)) m

let info_zprint m (pos:Globals.loc) = 
	ho_print true (fun x -> Lazy.force x) m

(* let devel_zprint msg (pos:Globals.loc) = *)
(* 	lazy_print (prior_msg pos) msg *)

(* let trace_zprint msg (pos:Globals.loc) =  *)
(* 	lazy_print (fun () -> " ") msg *)


let print_info prefix str (pos:Globals.loc) =
  let open Globals in
